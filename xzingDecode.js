module.exports = function (RED) {
  function xzingDecode(config) {
    ...
    let stringformats = [];
    if (node.DATA_MATRIX) {
      stringformats.push("DATA_MATRIX");
    }
    if (node.QR_CODE) {
      stringformats.push("QR_CODE");
    }
    if (node.PDF_417) {
      stringformats.push("PDF_417");
    }
    if (node.EAN_8) {
      stringformats.push("EAN_8");
    }
    if (node.EAN_13) {
      stringformats.push("EAN_13");
    }
    if (node.CODE_39) {
      stringformats.push("CODE_39");
    }
    if (node.CODE_128) {
      stringformats.push("CODE_128");
    }
    if (node.ITF) {
      stringformats.push("ITF");
    }
    if (node.RSS_14) {
      stringformats.push("RSS_14");
    }
    ...
    function decodeBarcode(img, formats, tryHarder) {
      if (!img || !img.bitmap) {
        throw new Error(
          "Image is not valid. Expected an object with a bitmap property."
        );
      }
    ...
    node.on("input", function (msg) {
        ...
        (err, value) => {
          if (err) {
            ...
            node.status({
              fill: "red",
              shape: "ring",
              text: "Unable to evaluate tryHarder",
            });
            node.done();
            return; //halt flow!
          } else {
      ...
      try {
        ...
        if (
          typeof data == "string" &&
          data.substr(0, 30).indexOf("base64") != -1 &&
          data.substr(0, 4).indexOf("data") == 0
        ) {
